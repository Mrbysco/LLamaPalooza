...
public class LLamaLootProvider extends LootTableProvider {
	...
	private static class LLamaEntityLoot extends EntityLootSubProvider {
		...
		protected Stream<EntityType<?>> getKnownEntityTypes() {
			return LLamaRegistry.ENTITY_TYPES.getEntries().stream().map(DeferredHolder::get);
		}
	}
		...
		public void generate(BiConsumer<ResourceLocation, LootTable.Builder> consumer) {
			consumer.accept(LLamaTables.GENERAL, LootTable.lootTable()
					.withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F))
							.name("main")
							.add(TagEntry.expandTag(ItemTags.DIRT).setWeight(10)
			...
			consumer.accept(LLamaTables.ORES, LootTable.lootTable()
					.withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F))
							.name("main")
							.add(TagEntry.expandTag(Tags.Items.ORES_COAL).setWeight(20)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 6.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_IRON).setWeight(18)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 5.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_COPPER).setWeight(15)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 5.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_GOLD).setWeight(10)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 4.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_LAPIS).setWeight(8)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 3.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_REDSTONE).setWeight(8)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 3.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_QUARTZ).setWeight(6)
									.apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 3.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_EMERALD).setWeight(5)
									.apply(SetItemCountFunction.setCount(ConstantValue.exactly(1.0F)))
							)
							.add(TagEntry.expandTag(Tags.Items.ORES_DIAMOND).setWeight(4)
									.apply(SetItemCountFunction.setCount(ConstantValue.exactly(1.0F)))
							)
					)
			);
